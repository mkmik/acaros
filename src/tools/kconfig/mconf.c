 ...
 *
 * Modified for use with acaros:
 * 24.02.03 Marko Mikulicic <marko@seul.org>
 *
 */
...
int mconf_main(int ac, char **av)
{
	...
	sym_calc_value(sym);
	sprintf(menu_backtitle, "Acaros Kernel v%s Configuration",
	...
	if (stat == 0) {
		conf_write(NULL);
		printf("\n\n"
			"*** End of Acaros kernel configuration.\n"
			"*** Check the 'config.mk' file for additional configuration.\n"
			"*** Next, you may run 'make', or 'make install'.\n\n");
	} else
