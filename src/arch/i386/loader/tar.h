...
/** returns the number of blocks given a number of bytes */
int tar_blocks_from_size(int size);

/** retunrs the size of a file in blocks */
int tar_file_blocks(struct tar_archive* file);

/** like tar_size but needs a pointer to tar_archive */
int tar_file_size(struct tar_archive* file);

/** returns the pointer to the file data */
void* tar_file_data(struct tar_archive* file);

/** returns a pointer to the next file, given a pointer
 * to a file */
struct tar_archive* tar_next_file(struct tar_archive* arch);

/** returns true if the pointer points to a valid tar file header */
int tar_is_valid_file(struct tar_archive* file);

/** returns a pointer to a file with the given name or null if it cannot
 * be found */
struct tar_archive* tar_find_file(struct tar_archive* arch, char* name);

void tar_test(struct tar_archive* arch, char *name);

// derived from GNU tar "tar.h"
